...
### "인기 영상 하나 주면 새 영상이 나오나?" — 정확한 답
**반만 맞음.** 원본 footage를 잘라 재편집하는 게 아니라, 원본은 *벤치마크*로만 쓰고
**대본·음성·영상이 전부 새로 만들어진 오리지널**이 산출됨.
```
인기 쇼츠(입력) → [분석] 주제·구성·훅만 추출
              → [대본] 같은 주제로 내 관점 새 대본 (Claude)
              → [음성] 내 나레이션 (ElevenLabs)
              → [비주얼] 라이선스 클립 + 내 화면/차트
              → [조립] CapCutAPI → 새 영상
결과 = 같은 주제, 100% 새 재료의 오리지널 (Content ID·재사용정책 통과)
```
- 원본을 잘라 효과만 입혀 "달라 보이게" 하는 길은 **안 감**(잡히고 손해).
- 자동화는 **조립까지**, 최종 손질·렌더는 사람 → 완전 무인 버튼 아님.
...
| 3 TTS | script | 텍스트→음성 | `narration.mp3` | **ElevenLabs (확정)** | ★☆☆ |
...
| 단계 | 항목 | 비용 | 비고 |
|------|------|------|------|
| 3 음성 | **ElevenLabs** | 🔴 이미 결제 중 (고정비) | 글자수 비례, voice_id 고정 권장 |
| 2 대본 | Claude API | 🟡 영상당 수십 원 | 직접 작성 시 0원 |
| 7 마감 | 캡컷 | 🟡 무료 / Pro 선택 | 무료로도 export 대체로 가능 |
| 6 조립 | CapCutAPI | 🟢 무료 | 오픈소스 |
| 5 자막 | faster-whisper(로컬) | 🟢 무료 | OpenAI Whisper API 쓰면 유료 |
| 4 비주얼 | Pexels/Pixabay | 🟢 무료 | 상업적 사용 가능 |
| 1 벤치마킹 | YouTube Data API | 🟢 무료 | 할당량 내 |
| — | 클립 서빙/컴퓨팅 | 🟢 무료 | localhost + 내 PC |

**결론**: 새로 나가는 돈은 사실상 **Claude 대본값(소액)뿐**. ElevenLabs는 이미 내는
고정비. 나머지는 0원. → 영상 1개 추가 비용 ≈ 거의 0원.

> 가격·약관은 자주 바뀜. 착수 전 각 서비스 최신 요금 확인.
> ElevenLabs 음성의 **상업적 사용/소유권 조건은 플랜마다 다르므로** 수익화 전 약관 확인.

---

## 6. 미해결 과제 (구현 전 정해야 할 것)
